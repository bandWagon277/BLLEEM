...
# 改进后的 summarize_results，支持计算 rank correlation
def summarize_results(results, true_ranks):
    ...
    for result in results:
        ...
        # 计算 rank correlation
        common_keys = [k for k in rank_dict if k in true_ranks]
        predicted_ranks = [rank_dict[k] for k in common_keys]
        ground_truth_ranks = [true_ranks[k] for k in common_keys]
        if len(predicted_ranks) > 1:
            rho, _ = spearmanr(predicted_ranks, ground_truth_ranks)
            correlation_list.append(rho)

    # 均值和标准差
    rank_summary = {
        key: {"mean": np.mean(ranks), "sd": np.std(ranks)}
        for key, ranks in rank_accumulator.items()
    }

    mse_array = np.array(mse_list)
    mse_summary = {"mean": mse_array.mean(), "sd": mse_array.std()}

    correlation_array = np.array(correlation_list)
    correlation_summary = {"mean": correlation_array.mean(), "sd": correlation_array.std()}

    return rank_summary, mse_summary, correlation_summary
...
if __name__ == "__main__":
    ...
    result_all = []
    # ---- Generate true ranks (only once) ----
    para_true = (alpha * delta)[:, np.newaxis] * w
    flat_para = para_true.flatten()
    ranks = (-flat_para).argsort().argsort()  # 从小到大排名
    true_ranks = {
        f"x_{i}_{j}": int(ranks[i * T + j]) + 1
        for i in range(P) for j in range(T)
    }
    ...
    for i in range(10):
        ...
        flat_para = para.flatten()
        ranks = (-flat_para).argsort().argsort()
        ...
        MSE = 0  # Replace with actual MSE if needed
        result = {"Rank": importance_dict, "MSE": MSE}
        result_all.append(result)

    # ---- Summarize ----
    rank_summary, mse_summary, correlation_summary = summarize_results(result_all, true_ranks)

    # ---- Print ----
    print("MSE summary:", mse_summary)
    print("Rank correlation summary:", correlation_summary)
    print("Rank summaries:")
    for k in rank_summary:
    ...
    rank_summary, mse_summary, correlation_summary = summarize_results(result_1, true_ranks)
