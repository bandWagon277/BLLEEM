...
class BLLEEMGibbsSampler:
    ...
    def sample_weights(self):
        #Use stick-breaking to sampling w_jt
        for j in range(self.P):
            if self.delta[j] == 0:  # Unselected variables are kept evenly weighted
                continue
                
            # Save the old weights for calculating the acceptance probability
            w_old = self.w[j].copy()
            
            # Samping new stick-breaking varaible
            v = np.zeros(self.T-1)
            for t in range(self.T-1):
                # Using Jeffreys prior
                a = 0.5  # tau_t
                b = 0.5 * (self.T - t - 1)  # sum_{s>t} tau_s
                v[t] = stats.beta.rvs(a, b)
                
            # calculate new weight
            w_new = self.stick_breaking(v)
            
            # compute acceptance ratio（MH step）
            log_ratio = self._log_likelihood_ratio(j, w_old, w_new)
            if np.log(np.random.uniform()) < log_ratio:
                self.w[j] = w_new

    def _log_likelihood_ratio(self, j, w_old, w_new):
        #Compute the log-likelihood ratio of the weight update
        X_centered = self.X[:, j] - self.m_jt[j]  # (n, T)
        effect_old = self.alpha[j] * np.dot(X_centered, w_old)
        effect_new = self.alpha[j] * np.dot(X_centered, w_new)
        
        residual_old = self.y - effect_old - self.Z @ self.beta
        residual_new = self.y - effect_new - self.Z @ self.beta
        
        log_ratio = -0.5/self.sigma_eps**2 * (np.sum(residual_new**2 - residual_old**2))
        return log_ratio
    ...
    def _log_posterior_vjt(self, v, j, t):
        ...
        #tau_t = 1
        tau_t = np.maximum(self.tau[t],1)
        #sum_tau_st = (self.T - t)
        sum_tau_st = np.maximum(np.sum(self.tau[t:]),(self.T - t))
        log_prior = (
    ...
    def _sample_vjt_ars(self, j, t):
        """Use Adaptive Rejection Sampling (ARS) to sample v_{jt} using arspy."""
        lb, ub = 1e-6, 1 - 1e-6  # Ensure v_{jt} is strictly within (0,1)
        ...
        # Use arspy's adaptive_rejection_sampling function
        v_jt_sampled = arspy.ars.adaptive_rejection_sampling(
            logpdf=logpdf,    # Log probability density function
            a=lb,             # Lower bound for initialization
            b=ub,             # Upper bound for initialization
            domain=(lb, ub),  # Domain of the function
            n_samples=1       # Number of samples to draw
        )

        return v_jt_sampled[0]  # Extract single sample from list
