...
class BLLEEMGibbsSampler:
    def __init__(self, y, X, Z, P, T, L, n_iter=1000, device='cuda'):
        ...
        # 模型参数（显式启用梯度）
        self.beta = torch.ones(L + 1, device=self.device, requires_grad=True)
        self.alpha = torch.zeros(P, device=self.device, requires_grad=True)
        ...
        self.v = torch.zeros((P, T), device=self.device)
        w_rows = []
        for j in range(self.P):
            ...
            # 生成单行权重
            w_row = self.stick_breaking(v)
            w_rows.append(w_row)
        
        # 合并所有行并启用梯度
        self.w = torch.stack(w_rows, dim=0).to(self.device).requires_grad_(True)

    def _initialize_weights(self):
        w = torch.zeros((self.P, self.T), device=self.device, requires_grad=True)
        for j in range(self.P):
            # 确保Beta参数在GPU
            v = torch.tensor(
                [np.random.beta(self.tau[t].item(), self.tau[t+1:].sum().item())
    ...
    def stick_breaking(v):
        ...
        for t in range(len(v)):
            w[t] = v[t] * remaining
            remaining = remaining * (1 - v[t])  # 避免原地操作
        w[-1] = remaining
        return w
    ...
    def pyro_model(self, j, t):
        ...
        # 确保先验参数是GPU张量
        tau_t = torch.tensor(1.0, device=self.device)
        sum_tau_st = torch.tensor(self.T - t, device=self.device)
        ...
        pyro.sample(
            f"obs_{j}_{t}",
            dist.Normal(0, self.sigma_eps).expand(residual_new.shape),
            obs=residual_new
        )
    ...
    def sample_weights_ars(self):
        ...
        with torch.no_grad():
            for t in range(self.T - 1):
                alpha = self.tau[t].to(self.device)
                beta = self.tau[t+1:].sum().to(self.device)
